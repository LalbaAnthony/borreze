...
// logs
$log = "Access to modifier_menu_cantine page \n";
write_in_logs($log);
...
$menuDAO = new MenuDAO();
$menuToModify = $menuDAO->findOneMenuById($idMenu);
...
if ($modifier) {
    ...
    if (!isset($errorMessage)) {
        ...
        if ($menuDAO->update($menu) === False) die("Erreur : les contenus ne font pas la bonne longeur, t'aurais pas essayé de bidouiller le formulaire ?");
...
<head>
    ...
    <title>Modifier un menu - Borrèze</title>
    ...
    <form action="<?= $_SERVER["PHP_SELF"] ?>" method="GET">
        <div class="my-2 w-25">
            <div class="d-flex justify-content-start">
                ...
                <input class="form-control" type="date" name="lundiDate" value="<?= $menuToModify->get_lundiDate() ?>" required />
            </div>
        </div>
            ...
            <tr>
                ...
                foreach (array('lundi', 'mardi', 'mercredi', 'jeudi', 'vendredi') as $dayLabel) {
                    if ($dayLabel != "mercredi") {
                        $methodeGetRepas = 'get_' . $dayLabel . "Repas";
                        $methodeGetIsFerie = 'get_is' . ucfirst($dayLabel) . "Ferie";
                        echo "<td>";
                        echo "<textarea minlength='10' maxlength='100' placeholder='Menu du " . $dayLabel . "' name='" . $dayLabel . "Repas' id='" . $dayLabel . "' cols='30' rows='10' class='form-control'>" . $menuToModify->$methodeGetRepas() . "</textarea>";
                        echo "<label class='mx-1' for='is" . ucfirst($dayLabel) . "Ferie'>Jour férié ?</label>";
                        if ($menuToModify->$methodeGetIsFerie() === 1) {
